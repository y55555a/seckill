...
var seckill = {
    ...
    myCountDown : function (seckillId, nowTime, startTime, endTime) {
        ...
        if (nowTime > endTime) {
            ...
            seckillBox.countdown(killTime, function (event) {       //function回调函数
                //时间格式
    ...
    handlerSeckill: function (Id, node) {
        //获取秒杀地址,控制显示器,执行秒杀
        console.log("seckill begin");

        node.hide().html('<button class="btn btn-primary btn-lg" id="killBtn">开始秒杀</button>').show();

        $.post(seckill.URL.exposer(), {seckillId : Id}, function (result) {
            //在回调函数种执行交互流程
            if (result && result['success']) {
                ...
                if (exposer['exposed']) {
                    ...
                    $('#killBtn').one('click', function () {
                        ...
                        //2.发送秒杀请求执行秒杀
                        $.post(killUrl, {seckillId : Id, md5 : md5, phone:inputPhone}, function (result) {
                            if (result && result['success']) {
